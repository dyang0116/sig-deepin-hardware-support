...
SCRIPT_VERSION="20220822"

DRV_NAME="rtl8188gtvu"
DRV_VERSION="5.11.5.4"
...
then
	/usr/src/backport-wifi-1.2.3/realtek/rtl8188gtvu/remove-driver.sh
fi

echo "Starting installation..."
# the add command requires source in /usr/src/${DRV_NAME}-${DRV_VERSION}
echo "Copying source files to: /usr/src/${DRV_NAME}-${DRV_VERSION}"
cp -rf /usr/src/backport-wifi-1.2.3/realtek/rtl8188gtvu /usr/src/${DRV_NAME}-${DRV_VERSION}
