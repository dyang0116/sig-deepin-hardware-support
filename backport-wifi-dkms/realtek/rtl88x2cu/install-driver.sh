...
then
	/usr/src/backport-wifi-1.2.3/realtek/rtl88x2cu/remove-driver.sh
fi

echo "Starting installation..."
# the add command requires source in /usr/src/${DRV_NAME}-${DRV_VERSION}
echo "Copying source files to: /usr/src/${DRV_NAME}-${DRV_VERSION}"
cp -rf /usr/src/backport-wifi-1.2.3/realtek/rtl88x2cu /usr/src/${DRV_NAME}-${DRV_VERSION}
